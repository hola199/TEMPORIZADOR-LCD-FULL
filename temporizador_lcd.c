...
void mostrar_segundos(){     //funcion para visualizar segundos
     switch(N0_Temp){
      case 0:
...
void mostrar_minutos(){      //funcion para visualizar minutos
     switch(N0_Temp){
      case 0:
...
void mostrar_horas(){      //funcion para visualizar horas
     switch(N0_Temp){
      case 0:
      decenas=T0.Hora/10;
      unidades=T0.Hora%10;
...
void mostrar_segundos_temp(){     //funcion para visualizar segundos (edicion)
     decenas=Temp.Segundo/10;
     unidades=Temp.Segundo%10;
     Lcd_Chr(2,14,(unidades+48));
     Lcd_Chr(2,13,(decenas+48));
}
void mostrar_minutos_temp(){      //funcion para visualizar minutos (edicion)
...
void mostrar_horas_temp(){      //funcion para visualizar horas  (edicion)
...
void parpadear_segundos_temp(){  // funcion para hacer parpadear los segundos
...
void parpadear_minutos_temp(){  //funcion para hacer parpadear los minutos
      mostrar_segundos_temp();
      mostrar_horas_temp();
...
void parpadear_horas_temp(){   //funcion para hacer parpadear las horas
     mostrar_segundos_temp();
     mostrar_minutos_temp();
...
void mostrar_puntos(){  //funcion para visualizar y hacer parpadear los dos puntos " : "
      if(activar==1){
        Lcd_Chr(2,12,58);
        Lcd_Chr(2,9,58);
      }
...
void cuenta_regresiva1(){  //configuracion/cuenta regresiva ( dispositovo1)
   if(T0.Segundo>0 || T0.Minuto>0 || T0.Hora>0){
      ...
      if(T0.Hora<0)T0.Hora=0;
   }
}

void cuenta_regresiva2(){  //configuracion/cuenta regresiva(dispositivo2)
    if(T1.Segundo>0 || T1.Minuto>0 || T1.Hora>0){
      ...
      if(T1.Hora<0)T1.Hora=0;
    }
}

void cuenta_regresiva3(){  //configuracion/cuenta regresiva (dispositivo 3)
    if(T2.Segundo>0 || T2.Minuto>0 || T2.Hora>0){
      ...
      if(T2.Hora<0)T2.Hora=0;
    }
}

void cuenta_regresiva4(){  //configuracion/cuenta regresiva ( dispositivo 4)
...
void leer_EEPROM(){ //tranferir los datos de la EEPROM a vaiables de conteo
...
void visualizar_N0_Timer(){
            ...
            switch(N0_Temp){
              ...
              if(DISPOSITIVO_1 == 1) habilitar_puntos = 1;
               else {
                habilitar_puntos = 0;
                activar = 1;
               }
               //visualizar el estado del dispositivo
              if(DISPOSITIVO_1==1) Lcd_Out(2,2,onn);
              else Lcd_Out(2,2,off);
              ...
              if(DISPOSITIVO_2 == 1) habilitar_puntos = 1;
               else habilitar_puntos = 0;
               
               if(DISPOSITIVO_2==1) Lcd_Out(2,2,onn);
                else {
                 Lcd_Out(2,2,off);
                 activar = 1;
                }
              ...
              if(DISPOSITIVO_3 == 1) habilitar_puntos = 1;
               else {
                habilitar_puntos = 0;
                activar = 1;
               }
               if(DISPOSITIVO_3==1) Lcd_Out(2,2,onn);
               else Lcd_Out(2,2,off);
              ...
              if(DISPOSITIVO_4 == 1) habilitar_puntos = 1;
               else {
                habilitar_puntos = 0;
                activar = 1;
               }
               if(DISPOSITIVO_4==1) Lcd_Out(2,2,onn);
               else Lcd_Out(2,2,off);
...
void Inc_Timer(){
             switch(selector){ //cada estado de "selector" decrementa una variable
               ...
               case 0:
                 pausa(); Temp.Segundo++;
                 if(Temp.Segundo > 59) Temp.Segundo = 0;
               break;

               case 1:
                 pausa(); Temp.Minuto++;
                 if(Temp.Minuto > 59) Temp.Minuto = 0;
               break;

               case 2:
                 pausa(); Temp.Hora++;
                 if(Temp.Hora > 23) Temp.Hora = 0;
               break;
             }
}
...
void Dec_Timer(){
            switch(selector){   // cada estado de "selector" decrementa una variable
              case 0:
                pausa(); Temp.Segundo--;
                if(Temp.Segundo < 0) Temp.Segundo = 59;
              break;

              case 1:
                pausa(); Temp.Minuto--;
                if(Temp.Minuto< 0) Temp.Minuto = 59;
              break;

              case 2:
                pausa(); Temp.Hora--;
                if(Temp.Hora < 0) Temp.Hora = 23;
              break;
            }
}
...
void alarma(){
             ...
             if(nn==1){
                 ...
                 if(alarma1) Lcd_Out(1,2,"Temporizador 1");
                else if(alarma2) Lcd_Out(1,2,"Temporizador 2");
                else if(alarma3) Lcd_Out(1,2,"Temporizador 3");
                else if(alarma4) Lcd_Out(1,2,"Temporizador 4");
...
void configuracion_interruptiones(){  //funcion para configurar y habilitar interrupciones
      ADCON1 = 0X0F;
      T0CON = 0XC6;
      INTCON = 0XA8;
      INTCON2 = 0X05;
...
void init_main(){    // configuracion de puertos
     TRISB=0XFF;
     TRISC=0X00;
     TRISE=0;
...
void interrupt(){
  ...
  if(T0IF_bit){ // interrupciones por el TMR0
     ...
     if(contador2 == 70){
      ...
      if(DISPOSITIVO_1==1 || DISPOSITIVO_2==1 || DISPOSITIVO_3==1 || DISPOSITIVO_4==1){
           contador++;
         if(contador == 50){
          if(habilitar_puntos == 1) activar = 0;
         }
         if(contador == 100){
          //decremento de segundos, segun lo indique el estado de cada dispositivo
           if(DISPOSITIVO_1==1) T0.Segundo--;
           if(DISPOSITIVO_2==1) T1.Segundo--;
           if(DISPOSITIVO_3==1) T2.Segundo--;
           if(DISPOSITIVO_4==1) T3.Segundo--;
           if(habilitar_puntos == 1) activar = 1;
            contador=0;
         }
       }
    ...
    //presentacion();
     ...
     while(1){
       ...
       while(programa == 0){
            ...
            mostrar_puntos();
            if(habilitar_EEPROM == 1) leer_EEPROM();
           ...
           if(alarma1==0&&alarma2==0&&alarma3==0&&alarma4==0) {
            ...
            //Encender dispositivo 1
             if(DISPOSITIVO_1 == 0 && N0_Temp == 0){
              if(estado1 == 1)habilitar_EEPROM=1;
               if(init_timer){
                   while(init_timer);
                   DISPOSITIVO_1 = 1;
               }
             ...
             if(DISPOSITIVO_2 == 0 && N0_Temp == 1){
              if(estado2 == 1)habilitar_EEPROM=1;
              if(init_timer){
                  while(init_timer);
                  DISPOSITIVO_2 = 1;
              }
             ...
             if(DISPOSITIVO_3 == 0 && N0_Temp == 2){
               if(estado3 == 1)habilitar_EEPROM=1;
              if(init_timer){
                  while(init_timer);
                  DISPOSITIVO_3 = 1;
              }
             ...
             if(DISPOSITIVO_4 == 0 && N0_Temp == 3){
              if(estado4 == 1)habilitar_EEPROM=1;
              if(init_timer){
                  while(init_timer);
                  DISPOSITIVO_4 = 1;
              }
       ...
       while(programa == 1){
        ...
        if(h==0){
          ...
          if(Inc){ // regresar a, configurar la alarma
            while(Inc);
            Lcd_Cmd(_lCD_CLEAR);
            h=1;
          }

          if(OK){ // Confirmacion para configurar los temporizadores
           while(OK);
            Lcd_Cmd(_lCD_CLEAR);
             programa = 2;
          }
       ...
       while(programa == 3 ){
          ...
          if(N0_Temp == 0)Lcd_Out(1,3,disp_1);
          if(N0_Temp == 1)Lcd_Out(1,3,disp_2);
          if(N0_Temp == 2)Lcd_Out(1,3,disp_3);
          if(N0_Temp == 3)Lcd_Out(1,3,disp_4);
         ...
         if(Dec){  //decender
          ...
          switch(selector){
              case 0: parpadear_segundos_temp(); break;
              case 1: parpadear_minutos_temp();  break;
              case 2: parpadear_horas_temp();    break;
          }
